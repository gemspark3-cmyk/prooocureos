...
const releaseData = {
    ...
    "en": {
        ...
        "AIP-101": { "title": "NLP Engine & Multi-Language Support", "description": "RFQ translation and analysis in 72 languages.", "cat1": "Foundation", "item1": "First successful demand-supplier matching with NLP.", "item2": "Successful demand-supplier matching with NLP." }
    }
};
...
for (const lang of allLangs) {
    ...
    // Ensure releaseItems exists and is fully populated
    // We use the language specific data if available, otherwise fallback to EN
    const baseItems = releaseData[lang] || releaseData["en"];
    
    // Merge: we want to ensure all keys from EN are present in every language
    const finalItems = {};
    const enItems = releaseData["en"];
    
    for (const version in enItems) {
        finalItems[version] = baseItems[version] || enItems[version];
    }
    
    data.changelog.releaseItems = finalItems;
    
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2), 'utf8');
    console.log(`Fully rebuilt release items for ${lang}.json`);
}
