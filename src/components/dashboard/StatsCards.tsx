  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div 
        ...
        className={`glass-premium p-6 rounded-[2rem] border border-white/5 flex flex-col gap-4 group hover:border-blue-500/20 transition-all relative overflow-hidden ${stats.activeRfqs === 0 ? 'cursor-pointer' : ''}`}
      >
        ...
        <div className="relative z-10">
          ...
          {stats.activeRfqs === 0 ? (
            ...
            <h4 className="text-3xl font-black text-white italic mt-1">{stats.activeRfqs}</h4>
          )}
        </div>
        <div className="absolute top-0 right-0 w-24 h-24 bg-blue-600/5 blur-[40px] -mr-12 -mt-12 rounded-full" />
      </div>

      <div className="glass-premium p-6 rounded-[2rem] border border-white/5 flex flex-col gap-4 group hover:border-emerald-500/20 transition-all relative overflow-hidden">
        <div className="flex justify-between items-start relative z-10">
        ...
        <div className="relative z-10">
          ...
          <h4 className="text-3xl font-black text-white italic mt-1">{formatCurrency(Number(stats.estimatedSavings) || 0, profileData?.preferred_currency || 'USD')}</h4>
        </div>
        <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-600/5 blur-[40px] -mr-12 -mt-12 rounded-full" />
      </div>

      <div className="glass-premium p-6 rounded-[2rem] border border-white/5 flex flex-col gap-4 group hover:border-purple-500/20 transition-all relative overflow-hidden">
        <div className="flex justify-between items-start relative z-10">
        ...
        <div className="relative z-10">
          ...
          <h4 className="text-3xl font-black text-white italic mt-1">{String(stats.avgResponseTime).replace(/saat/gi, '').trim()} <span className="text-xs text-zinc-500 lowercase">{t('responseTimeUnit')}</span></h4>
        </div>
        <div className="absolute top-0 right-0 w-24 h-24 bg-purple-600/5 blur-[40px] -mr-12 -mt-12 rounded-full" />
      </div>

      <div className="glass-premium p-6 rounded-[2rem] border border-white/5 space-y-3 relative overflow-hidden group hover:border-amber-500/20 transition-all">
        ...
        <div className="flex justify-between items-center relative z-10">
          <span className="text-[11px] font-black text-white italic">{formatCurrency(stats.totalSpent, profileData?.preferred_currency || 'USD')}</span>
